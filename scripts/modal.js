...
function renderUsuarioOnModal(usuario) {
  ...
  <h3 class="px-5 mt-4 mb-2 text-center">Eu, <b>${usuario.nome}</b>, de <b>${
    usuario.idade + " Anos"
  }</b> , residindo em <b>${usuario.cidade}</b> com Cep <b>${
    usuario.cep
  }</b>, no Bairro <b>${usuario.bairro}</b>, na Rua <b>${
    usuario.rua
  }</b> e número <b>n°${usuario.numero}</b>,
  </b> vendo minha alma no valor de um Pastelzinho com Coca;<p/></h3>
  <h3 class="px-5 mt-3 text-center">Parágrafo único: os Hobbies <b>${usuario.hobbies.join(
    ", "
  )}</b> que eram atribuídos ao <b>Cliente</b> passam a ser do <b>Fornecedor</b>;</h3>
